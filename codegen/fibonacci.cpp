...
static Function *CreateFibFunction(Module *M, LLVMContext &Context, Type* dataType) {
  // Create the fib function and insert it into module M. This function is said
  // to return an int and take an int parameter.
  Function *FibF = cast<Function>(M->getOrInsertFunction("fib", dataType,dataType,nullptr));

  // Add a basic block to the function.
  BasicBlock *BB = BasicBlock::Create(Context, "EntryBlock", FibF);

  // Get pointers to the constants.
  Value *One = ConstantInt::get(dataType, 1);
  Value *Two = ConstantInt::get(dataType, 2);
