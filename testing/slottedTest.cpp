...
#include "../dbms/DBMS.hpp" // include your stuff here
...
int main(int argc, char** argv) {
   ...
   // TODO ...
   SPSegment sp = SPSegment(bm, 7, 0); //hardcoded to segment 7 with initial size 0
   Random64 rnd;

   string blub = "mein ganze eigener toller string";
   TID meineTid = sp.insert(Record(blub.size()+1,blub.c_str()));
   // Insert some records
   ...
   Record rec = sp.lookup(meineTid);
   //cout << rec.getData() << endl;

   // Update some values ('usage' counter invalid from here on)
   for (unsigned i=0; i<maxUpdates; ++i) {
      // Select victim
      unordered_map<TID, unsigned>::iterator it = values.begin();
      advance(it, rnd.next()%(values.size()-1));
      TID tid = it->first ;
